...
class TrieNode {
 public:
  ...
  auto InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) -> std::unique_ptr<TrieNode> * {
    if (this->HasChild(key_char) || child->key_char_ != key_char) {
      return nullptr;
    }
...
class Trie {
 ...
 public:
  ...
  auto Insert(const std::string &key, T value) -> bool {
    ...
    for (char c : key) {
      if ((*cur)->HasChild(c)) {
        ...
        cur = (*cur)->InsertChildNode(c, std::make_unique<TrieNode>(c));
        if (cur == nullptr) {
          latch_.WUnlock();
          return false;
        }
   ...
   */
  auto CanRemove(const std::string &key, size_t i, std::unique_ptr<TrieNode> *cur, bool *ok) -> bool {
    if (i == key.size()) {
      *ok = (*cur)->IsEndNode();
  ...
  auto GetValue(const std::string &key, bool *success) -> T {
    ...
    *success = false;
    ...
    auto end = dynamic_cast<TrieNodeWithValue<T> *>((*cur).get());
    if (end != nullptr) {
      ret = (end)->GetValue();
      *success = true;
      latch_.RUnlock();
